...
description: "DeerFlow 通过 outbound WebSocket/polling 连接接入 8 个 IM 平台（+Buzz/Nostr），另有 Lark CLI 托管集成。所有频道共享同一套消息总线架构，但在 stream 策略上分裂为两派：增量流式（Feishu/DingTalk/Buzz）和阻塞等待（Slack/Telegram）。"
type: index
---
