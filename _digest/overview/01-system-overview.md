...
| 阶段 | Hook 点 | 参与 middleware（共 35 个中的代表） |
|------|--------|----------|
...
> 完整 35 个 middleware 的 hook 分配见 [middleware/03-catalog.md](../internals/middleware/03-catalog.md)。
...
| **Sandbox** | Tool Execute (lazy init) | `ensure_sandbox_initialized(runtime)` 包裹每个 sandbox tool | `ensure_sandbox_initialized_async` = `asyncio.to_thread` | [concepts/sandbox/abstract-interface-and-six-impls.md](../concepts/sandbox/abstract-interface-and-six-impls.md) |
...
5. 调用 `_build_middlewares()` 构建 35 个 middleware
