...
| `03-request-flow.md` | 完整请求生命周期：HTTP POST → SSE 返回 |

## SVG 全景图

| 文件 | 内容 |
|------|------|
| `01-logical-architecture.md` | 逻辑结构图：四层同心圆 + Agent Loop + Middleware Chain |
| `02-component-architecture.md` | 组件结构图：进程拓扑 + 依赖关系 |
| `03-startup-flow.md` | 启动流程图：6 阶段从命令到 agent.astream |
