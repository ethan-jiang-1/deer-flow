...
return create_agent(
    ...
    middleware=_build_middlewares(...),   # DeerFlow 的 35 个 middleware
...
LangChain 提供 6 种 hook 点 + 2 种 wrap 洋葱。DeerFlow 的 35 个 middleware 全部基于这些 hook：
...
**核心洞见**：DeerFlow 的 agent loop 不是自己写的循环，而是 **LangGraph Pregel 的 BSP superstep 机制 + LangChain 的 graph 节点/条件边 + DeerFlow 的 35 个 middleware 实现**。三层叠在一起，形成了完整的 agent 执行系统。
