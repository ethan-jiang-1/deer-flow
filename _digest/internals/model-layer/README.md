...
**回答的核心问题**：`create_chat_model()` 工厂怎么路由到 9 个自定义适配器（+ 2 个标准直通 = 11 条路由）、thinking/vision 的跨 provider 适配策略、streaming 的 chunk 归一化与 SSE 事件模型。
...
| 文件 | 内容 |
|------|------|
| **00-overview.md** | 全景：`create_chat_model()` 工厂、ModelConfig schema、9 个 DeerFlow 适配器 + 标准 LangChain 直通路径、凭证加载 |
