...
- **run 之外的 checkpoint 写入统一走 `services.reserve_checkpoint_write()`**（`app/gateway/services.py:98`）：它把进程内 `goal_thread_lock(thread_id)` 与 durable `checkpoint_write` reservation 组合起来。当前调用点共 5 处——手动 compaction（`threads.py:1417`）、`POST /threads/{id}/state`（`threads.py:1548`）、`PUT /threads/{id}/goal`（`threads.py:1353`，含为缺失 goal 按需创建 checkpoint）、`DELETE /threads/{id}/goal`（`threads.py:1374`）、以及 `/history` 的归因回写后台任务（`threads.py:1659`）。已有 run 会挡住这些写入，该 reservation 也挡住新的 reject/interrupt/rollback run，跨 worker 成立；冲突/租约丢失翻译成 `ConflictError` → 409。
...
| 4 | `RunStore.delete_by_thread()`（SQL 实现 `RunRepository`，memory 实现 `MemoryRunStore`；这是**可选能力**，不在 `RunStore` 抽象接口里，`threads.py:785` 用 `getattr` 探测、缺失即跳过）：只删 `operation_kind == "run"` 的历史 run 行——**第 1 步的 reservation 行必须活到 `reserve_thread_operation()` 退出**，否则 DELETE 会在请求中途丢掉自己的跨 worker 互斥；批量删故意不 bump change clock |
...
| 8 | 关闭该线程残留的 browser session（best-effort；Playwright 未安装时静默跳过） |

第 3-8 步（checkpoints 及之后）全部 **best-effort**：单项异常只记 debug 日志，不打断整体删除。第 2 步（文件系统）**不是** best-effort——`_delete_thread_data()` 把 `delete_thread_dir()` 的 `ValueError` 映射成 422、`FileNotFoundError` 视为成功、其余异常映射成 500，会直接中止删除（路由只额外捕获 `ConflictError`）。所有者身份在请求开头解析一次（文件系统 bucket、runs/events/feedback、thread_meta 用**同一个** `user_id`，`threads.py:756`），不允许各步骤各自解析作用域。

**边界（重要）**：本清理只删除"已存在的行"，**不负责阻止一个已被准入的写者在线程删除后把状态写回来**——那是 thread incarnation 的独立契约（`threads_meta.incarnation` 列已存在，但 v2.1.0 尚无 fencing 消费者，见 [01-run-manager.md](01-run-manager.md)）。upstream 在 `threads.py` 的 docstring 里明确把这两件事分开。
