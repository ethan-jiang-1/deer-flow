...
- **Skills 元数据与加载**：[`backend/packages/harness/deerflow/skills/AGENTS.md`](../../backend/packages/harness/deerflow/skills/AGENTS.md) 规定 SKILL.md 为"目录 + YAML frontmatter（name, description, license, allowed-tools, argument-hint, required-secrets）"；`load_skills()` 递归扫描 public / per-user custom / global integration / legacy 四个位置，**目录即包边界**："嵌套 SKILL.md 不再注册为运行时 skill"，安装器直接报错（`installer.py`：`nested SKILL.md is not allowed`），SkillScan 也把 `package-nested-skill-md` 列为 CRITICAL（`skills/skillscan/orchestrator.py`）。
