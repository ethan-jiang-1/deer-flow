...
- **Durable batch**：`config.yaml -> subagent_batches` + `batch_task`/`batch_status`/`cancel_batch` 工具 + 租约驱动的 `SubagentBatchService` + `/api/threads/{thread_id}/subagent-batches` REST API；per-item `acceptance_criteria` 持久化（migration 0021）并由 `batch_acceptance.py` 用沙箱租约跑同一清单。仓储 API 契约（投影白名单 / 提交幂等 / claim 四步 / 方法表）见 [dual-threadpool-and-lifecycle.md](dual-threadpool-and-lifecycle.md) 的「仓储层 API 契约」一节。
