...
- **共享组件**（RFC #4741，#5089）：`identity.py` 的 `derive_sandbox_scope_token`（keyword-only、SHA-256/16-hex 兼容性契约）+ `acquire_serialization.py` 的有界 per-key 锁表与专用 bounded executor。
