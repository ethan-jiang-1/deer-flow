...
>
> 🔄 同步 #7（v2.1.0 = 345f08be）：v2.1.0 相对 rc0 的实质源码改动不涉及本 FAQ 引用的任何行号出处，故上表按同步 #6 复核的行号继续有效；本轮另抽查了清单外引用，修正了个别早已漂移的行号/计数与失效的 digest 交叉链接；各文件中的 `v2.1.0-rc0` 标记保留为历史同步记录。
